...
```
Создать файл `.env` в каталоге `star_burger/`:
```sh
SECRET_KEY=django-insecure-0if40nf4nf93n4
YANDEX_API_KEY="API ключ кабинета разработчика Yandex"
```
...
Если планируете использовать логирование с помощью Rollbar, добавьте в .env ключ доступа(post_server_item):
...
## Docker, запуск проекта на локальном сервере

1. Cоздать файл .env в каталоге `dvmn-star-burger/` со следующими настройками:
...
- Если планируете использовать логирование с помощью Rollbar, добавьте в .env ключ доступа(post_server_item):
`ROLLBAR_ACCESS_TOKEN` — ключ сервиса логирования Rollbar
